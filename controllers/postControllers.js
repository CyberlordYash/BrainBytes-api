    ...
    thumbnail.mv(
      ...
      async (err) => {
        if (err) {
          ...
          //finding user and increment post count
  ...
  try {
    ...
    if (req.user.id == oldPost.creator) {
      if (!req.files) {
        ...
        //now we have to delete old thumbnail
        //so get old post from database
        ...
        //upload new thumbnail if no error
        const { thumbnail } = req.files;
        //checking file size
...
const deletePost = async (req, res, next) => {
  try {
    ...
    if (req.user.id == post.creator) {
      //deleting thumbnail from our uploads folder
      fs.unlink(
        ...
        async (err) => {
          if (err) {
            ...
            //after delete finding that user and reduce postcount
            const currentUser = await User.findById(req.user.id);
            const userPostCount = currentUser?.posts - 1;
            await User.findByIdAndUpdate(req.user.id, { posts: userPostCount });
